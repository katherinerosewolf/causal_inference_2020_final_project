...
```{r determining where schools get lost}
...
school_enrollment_in_dataset <- 
  sum(school_data_2016_17_sf$enr_total)

school_enrollment_dropped_lat_long <- 
  sum(school_enrollment_no_lat_long$enr_total)

limited_public_school_directory <- 
  public_school_directory_working %>% 
  select(cds_code_dir = cds_code, county, school, latitude, longitude) %>% 
  mutate(cds_school_code = str_sub(cds_code_dir, 
                                   start = -7))
  
# trying to match on counties and names
schools_matched_by_county_and_name <- 
  left_join(
    unique_schools_no_nonsec, 
    limited_public_school_directory, 
    by = c("county", "school", "cds_school_code"))

# trying to match on names
schools_matched_by_name <- 
  left_join(
    unique_schools_no_nonsec, 
    limited_public_school_directory, 
    by = c("school", "cds_school_code"))
