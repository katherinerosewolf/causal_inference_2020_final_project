...
```{r compare and compile epa and carb monitors into one unique monitor dataset}
...
```{r calculating monitor distances}
...
neighbors <- 
  st_nn(
    x = school_air_2016_17_sf, 
    y = all_monitors_no_duplicates_sf, 
    sparse = TRUE,
    k = 1, 
    maxdist = Inf,
    returnDist = TRUE, 
...
# make neighbor list into a vector
neighbor_id_vector <- 
  unlist(neighbors[1], 
         recursive = TRUE)
  
# make distance list into a vector
distance_vector <- 
  unlist(neighbors[2], 
         recursive = TRUE)
...
school_air_distance_2016_17_sf <- 
  bind_cols(
    ...
    nearest_neighbor = neighbor_id_vector
  )
...
plot(model)
plot(schools_without_pm_sf)


plot(air_cropped)
plot(schools_without_pm_sf, pch=16, cex=0.2, alpha = 0.8, col="black", add=TRUE)

  
(1527678+1496569+791) / 6218410

(1607723 + 1584723 + 926) / 6218410
