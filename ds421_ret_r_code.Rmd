...
school_enrollment_2009_10_working <- 
  read_delim(
    file = 
      file.path(
        "raw_data", 
        "school_enrollment_2009_10_raw.rdata"
        ),
    delim = "\t"
    )
...
school_enrollment_tab_reader <- 
  function(start_year_numeric){
    ...
    assign(variable_name, read_tibble)
...
# test function on all files
test <- 
  school_enrollment_tab_reader(2009)

load(
  file = 
    file.path(
      "intermediate_data", 
      "school_enrollment_2009_10_working.rdata"
    )
)
