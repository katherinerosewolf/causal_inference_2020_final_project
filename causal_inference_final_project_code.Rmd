...
# rename school thing
school_to_get_air_sf <-
  schools_sf

# extract values of air data at schools
school_to_get_air_sf$modeled_air_at_school <-
  raster::extract(
    air_cropped,
    school_to_get_air_sf
  )
...
plot(air_cropped)
plot(school_air_sf_nas, pch=16, cex=0.2, alpha = 0.8, col="black", add=TRUE)

mapview::mapView(school_air_na)
...
# look rows with nas
View(schools_all_data)

sum(is.na(schools_all_data$modeled_air_at_school))
sum(is.na(schools_all_data$air_district))
sum(is.na(schools_all_data$air_basin))
sum(is.na(schools_all_data$school_k_12_enrollment))
sum(is.na(schools_all_data$chronic_absenteeism_rate))
sum(is.na(schools_all_data$suspension_rate))
sum(is.na(schools_all_data$frpm_percent))
sum(is.na(schools_all_data$english_learners))
sum(is.na(schools_all_data$cost_per_pupil))
sum(is.na(schools_all_data$fte_school))
sum(is.na(schools_all_data$english_standard_met))
sum(is.na(schools_all_data$math_standard_met))
sum(is.na(schools_all_data$science_standard_met))


colnames(schools_all_data)
