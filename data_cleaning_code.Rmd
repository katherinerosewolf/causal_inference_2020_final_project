...
percent_fte_credentialed_by_school <- 
  ...
  mutate(cred_percent = cred_1/(cred_0 + cred_1)*100) %>% 
...
southern_ca <- c("Great Basin Valleys", 
                 "Mojave Desert", 
                 "Salton Sea", 
                 "San Diego County")

central_ca <- c("North Central Coast", 
                "South Central Coast", 
                "San Joaquin Valley")
...
school_air_sf_binned_basins <- 
  ...
  mutate(air_basin_binned = if_else(air_basin_binned %in% northern_ca,
                                   "Northern California", 
                                   air_basin_binned)) %>%
  mutate(air_basin_binned = if_else(air_basin_binned %in% southern_ca,
                                    "Southern California", 
                                    air_basin_binned)) %>% 
  mutate(air_basin_binned = if_else(air_basin_binned %in% central_ca,
                                    "Central California", 
                                    air_basin_binned)) %>% 
  mutate(air_basin_4_bins = air_basin_binned) %>% 
  mutate(air_basin_4_bins = if_else(air_basin_4_bins %in% southern_ca | 
                                      air_basin_4_bins == "South Coast",
                                    "Southern California", 
                                    air_basin_4_bins))   
...
ethnicity_grade_by_school <- 
  ...
  dplyr::select(cds_code, school_k_12_enrollment, sort(names(.)))  # sort columns
...
school_air_ethnicity_gender <- 
  ...
  left_join(ethnicity_grade_by_school, 
            by = "cds_code") %>% 
  dplyr::select(-c(cds_school_code, 
            cds_school_code_dir, 
            county_dir))
...
colnames(complete_cases_mapped)

str(complete_cases_mapped$math_standard_met)

plot(full_data_analysis_variables["math_standard_met"], pch=16, cex=0.4, 
     xlab=NULL, ylab=NULL, main=NULL, pal=brewer.pal(10, "RdYlGn"))
