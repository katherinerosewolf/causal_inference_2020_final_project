...
# correlations between predictors
  predictor_corrs <- Hmisc::rcorr(as.matrix(dataset_complete_cases[,c(3,num_cols)]))$r
...
# practice run of function
  ...
  # run function
    names(sub_data)
    exclude_vars_analysis <- c("cds_code","air_district","white_percent")
    out <- run.tmle(Y="math_standard_met", A="pm25_12_plus", ObsData=sub_data[,!(names(sub_data) %in% exclude_vars_analysis)], SL.library=SL.library) 
    ...
    est*100
# the real thing
  out <- run.tmle(Y="math_standard_met", A="pm25_12_plus", ObsData=dataset_complete_cases[,!(names(dataset_complete_cases) %in% exclude_vars_analysis)], SL.library=SL.library) 
  est <- out$estimates
  est*100
   
# run with ltmle for comparison
  sub_data2 <- sub_data[,!(names(sub_data) %in% exclude_vars_analysis)] # remove percent white too
  names(sub_data2)
