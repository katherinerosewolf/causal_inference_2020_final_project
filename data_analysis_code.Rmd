...
data_for_little_table_one <- 
  ...
  mutate("PM25" = factor(pm25_12_plus), 
         ...
         "Air basin" = air_basin,
         "asian_filipino_percent" = round(asian_filipino_percent*100, 2),
         "Asian (%)" = round(asian_percent*100, 2),
         "Filipino (%)" = round(filipino_percent*100, 2),
         "Hispanic/Latino (any race) (%)" = round(hisp_latinx_percent*100, 2),
         "White (%)" = round(white_percent*100, 2),
         "two_none_aian_pi_af_am_percent" = round(two_none_aian_pi_af_am_percent*100, 2),
         "African American (%)" = round(afr_am_percent*100, 2),
         "American Indian or Alaska Native (%)" = round(aian_percent*100, 2),
         "Pacific Islander (%)" = round(pacific_islander_percent*100, 2),
         "Two or more races (not Hispanic/Latino) (%)" = round(two_plus_percent*100, 2),
         "Race/ethnicity not reported (%)" = round(not_reported_percent*100, 2),
...
write_csv(hi, path = file.path("output_for_writups", "table_one_raw.csv"))
