...
load(file = "dataset_complete_cases_forget_funding.rdata")
...
# specify SL library
  listWrappers()
  SL.library <- c("SL.mean","SL.glm","SL.glm.interaction","SL.randomForest","SL.rpartPrune","SL.biglasso")
  other_options <- c("SL.glm.interaction","SL.randomForest","SL.polymars","SL.rpartPrune","SL.biglasso")
# SS, IPTW, & TMLE function
  run.tmle <- function(Y,A,ObsData, SL.library, id=NULL) {
    ...
    # Update the initial estimator of Qbar_0(A,W)
    #------------------------------------------
      logitUpdate<- glm(ObsData[,Y] ~ -1 +offset(qlogis(QbarAW)) + H.AW, family='gaussian')
      epsilon <- logitUpdate$coef
...
# practice run of function
  ...
  # run function
    out <- run.tmle(Y="math_standard_met", A="pm25_12_plus", ObsData=sub_data[,-c(1,4)], SL.library=SL.library) 
...
# run with ltmle for comparison
  sub_data2 <- sub_data[,-c(1,4)]
  sub_data2 <- sub_data2[,c(names(sub_data2[3:11]),"math_standard_met","pm25_12_plus")]
