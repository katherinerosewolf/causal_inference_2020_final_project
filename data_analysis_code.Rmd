...
run.tmle.katie <- function(ObsData, SL.library, adjustment_set, id = NULL){
...
out <- run.tmle.katie(ObsData = ObsData,
...
for(b in 1:B){
  ...
  estimates[b,]<- run.tmle.katie(ObsData = bootData, SL.library=SL.library, id=bootData$id)$estimates

  # keep track of the iterations completed
  print(b)
}

colnames(estimates)<-c("SimpSubs", "IPTW.HT", "IPTW", "TMLE")
