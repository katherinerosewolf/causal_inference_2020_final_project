...
run.tmle.katie <- function(ObsData, 
  ...
  # 0.025
  H.AW.025.975 <- as.numeric(ObsData$A==1)/gHat1W_025_975 - 
    as.numeric(ObsData$A==0)/gHat0W_025_975
  
  # also want to evaluate the clever covariates at A=1 and A=0 for all subjects
  H.1W.025.975 <- 1/gHat1W_025_975
  H.0W.025.975 <- -1/gHat0W_025_975
  
  # 0.05
  H.AW.05.95 <- as.numeric(ObsData$A==1)/gHat1W_05_95 - 
  as.numeric(ObsData$A==0)/gHat0W_05_95
  
  H.1W.05.95 <- 1/gHat1W_05_95
  H.0W.05.95 <- -1/gHat0W_05_95
  
  # 0.10
  H.AW.10.90 <- as.numeric(ObsData$A==1)/gHat1W_10_90 - 
  as.numeric(ObsData$A==0)/gHat0W_10_90
    
  H.1W.10.90 <- 1/gHat1W_10_90
  H.0W.10.90 <- -1/gHat0W_10_90
  
  # Update the initial estimator of Qbar_0(A,W)

  # 0.025!
  logitUpdate_025_975 <- glm(ObsData$Y ~ -1 + 
                               offset(qlogis(QbarAW)) + 
                               H.AW.025.975,
                     family='quasibinomial')
  epsilon_025_975 <- logitUpdate_025_975$coef
  
  QbarAW.025.975.star <- plogis(qlogis(QbarAW) + epsilon_025_975*H.AW.025.975)
  Qbar1W.025.975.star <- plogis(qlogis(Qbar1W) + epsilon_025_975*H.1W.025.975)
  Qbar0W.025.975.star <- plogis(qlogis(Qbar0W) + epsilon_025_975*H.0W.025.975)
  
  # 0.05!
  logitUpdate_05_95 <- glm(ObsData$Y ~ -1 + 
                             offset(qlogis(QbarAW)) + 
                             H.AW.05.95,
                     family='quasibinomial')
  epsilon_05_95 <- logitUpdate_05_95$coef
  
  QbarAW.05.95.star <- plogis(qlogis(QbarAW) + epsilon_05_95*H.AW.05.95)
  Qbar1W.05.95.star <- plogis(qlogis(Qbar1W) + epsilon_05_95*H.1W.05.95)
  Qbar0W.05.95.star <- plogis(qlogis(Qbar0W) + epsilon_05_95*H.0W.05.95)
  
  # 0.10!
  logitUpdate_10_90 <- glm(ObsData$Y ~ -1 + 
                             offset(qlogis(QbarAW)) + 
                             H.AW.10.90,
                     family='quasibinomial')
  epsilon_10_90 <- logitUpdate_10_90$coef
  
  QbarAW.10.90.star <- plogis(qlogis(QbarAW) + epsilon*H.AW.10.90)
  Qbar1W.10.90.star <- plogis(qlogis(Qbar1W) + epsilon*H.1W.10.90)
  Qbar0W.10.90.star <- plogis(qlogis(Qbar0W) + epsilon*H.0W.10.90)
  
  #------------------------------------------
  # Estimate Psi(P_0)
  #------------------------------------------
  
  PsiHat.TMLE.trunc.025.975 <- mean(Qbar1W.025.975.star - Qbar0W.025.975.star)
  PsiHat.TMLE.trunc.05.95 <- mean(Qbar1W.05.95.star - Qbar0W.05.95.star)
  PsiHat.TMLE.trunc.10.90 <- mean(Qbar1W.10.90.star - Qbar0W.10.90.star)
  ...
  estimates <- data.frame(cbind(PsiHat.SS=PsiHat.SS, 
                                ...
                                PsiHat.IPTW.05.95, 
                                PsiHat.IPTW.10.90, 
                                PsiHat.TMLE.trunc.025.975, 
                                PsiHat.TMLE.trunc.05.95, 
                                PsiHat.TMLE.trunc.10.90))
...
save(run.tmle.katie, 
     file = file.path("run.tmle.katie.rdata"))
...
run.tmle.katie.para <- function(ObsData, 
  ...
  # output_list <- 
  #           list(estimates = estimates, 
  #                QbarSL_risk = cbind(QbarSL$cvRisk, QbarSL$coef), 
  #                gHatSL_risk = cbind(gHatSL$cvRisk, gHatSL$coef))
...
# this is the parallel equivalent of a seed (CHANGE EVERY TIME SO HELP YOU GAIA)
parallel::clusterSetRNGStream(cluster, 200)
...
definitive_katie_results <- run.tmle.katie.para(ObsData = ObsData,
                                               g.SL.library = g.SL.library,
                                               Q.SL.library = Q.SL.library,
                                               cluster = cluster)

# save here CHANGE THIS NAME
save(definitive_katie_results, 
     file = "definitive_katie_results_seed_200_parallel.Rdata")

definitive_katie_results
...
# # test one run
# thing <- run.tmle.katie.para(ObsData = little_data,
#                               g.SL.library = g.SL.library,
#                               Q.SL.library = Q.SL.library,
#                               cluster = cluster)
...
output_list <- vector(mode = "list", length = B)
...
for(b in 1:B){
  ...
  output_list[[b]] <- run.tmle.katie.para(ObsData = bootData,
                                          g.SL.library = g.SL.library,
                                          Q.SL.library = Q.SL.library,
                                          cluster = cluster,
                                          id = bootData$id)
...
save(output_list, file = 'little_run.Rdata')
...
# this is the parallel equivalent of a seed (CHANGE EVERY TIME SO HELP YOU GAIA)
parallel::clusterSetRNGStream(cluster, 7)
...
# save here CHANGE THIS NAME
save(estimates, file = "estimates_seed_100.Rdata")

load(file = file.path("estimates_seed_100.rdata"))
...
```{r KATIE trying revised library to improve g?}
...
# this is the parallel equivalent of a seed (CHANGE EVERY TIME SO HELP YOU GAIA)
parallel::clusterSetRNGStream(cluster, 12)
...
# save here CHANGE THIS NAME
save(estimates, file = "g_revised_estimates_seed_12.Rdata")
...
```{r calculating bootstrap CIs}

load(file = "definitive_katie_results_seed_200_parallel.rdata")
load(file = "estimates_all.rdata")
...
# Zquant <- qnorm(alpha/2, lower.tail=F)
# 
# CI.normal = c(definitive_katie_results$estimates$PsiHat.SS -
#                 Zquant*sd(estimates_all[,"SimpSubs"]),
#               definitive_katie_results$estimates$PsiHat.SS +
#                 Zquant*sd(estimates_all[,"SimpSubs"]))

# CI for SS assuming a normal dist & then using quantiles
create.CI(pt=definitive_katie_results$estimates$PsiHat.SS,
          boot=estimates_all[,"SimpSubs"])

# CI for IPTW assuming a normal dist & then using quantiles
create.CI(pt=definitive_katie_results$estimates$PsiHat.IPTW,
          boot=estimates_all[,"IPTW"])

# CI for IPTW.HT assuming a normal dist & then using quantiles
create.CI(pt=definitive_katie_results$estimates$PsiHat.HW.IPTW,
          boot=estimates_all[,"IPTW.HT"])

# CI for TMLE assuming a normal dist & then using quantiles
create.CI(pt=definitive_katie_results$estimates$PsiHat.TMLE,
          boot=estimates_all[,"TMLE"])

# CI for IPTW 0.025 truncation assuming a normal dist & then using quantiles
create.CI(pt=definitive_katie_results$estimates$PsiHat.IPTW.025.975,
          boot=estimates_all[,"PsiHat.IPTW.025.975"])

# CI for IPTW 0.05 truncation assuming a normal dist & then using quantiles
create.CI(pt=definitive_katie_results$estimates$PsiHat.IPTW.05.95,
          boot=estimates_all[,"PsiHat.IPTW.05.95"])

# CI for IPTW 0.10 truncation assuming a normal dist & then using quantiles
create.CI(pt=definitive_katie_results$estimates$PsiHat.IPTW.10.90,
          boot=estimates_all[,"PsiHat.IPTW.10.90"])

# CI for TMLE 0.025 truncation assuming a normal dist & then using quantiles
create.CI(pt=definitive_katie_results$estimates$PsiHat.TMLE.trunc.025.975,
          boot=estimates_all[,"PsiHat.TMLE.trunc.025.975"])

# CI for TMLE 0.05 truncation assuming a normal dist & then using quantiles
create.CI(pt=definitive_katie_results$estimates$PsiHat.TMLE.trunc.05.95,
          boot=estimates_all[,"PsiHat.TMLE.trunc.05.95"])

# CI for TMLE 0.10 truncation assuming a normal dist & then using quantiles
create.CI(pt=definitive_katie_results$estimates$PsiHat.TMLE.trunc.10.90,
          boot=estimates_all[,"PsiHat.TMLE.trunc.10.90"])


# simple mean: areas with pm above 12 lose 2.03 percentage points of passage
mean_function <- function(ObsData){
  mean(ObsData$Y[ObsData$A == 1] - ObsData$Y[ObsData$A == 0])
}
...
# data frame for estimates based on the boot strap sample
meanies <- data.frame(matrix(NA, nrow = B, ncol = 1))
...
for(b in 1:B){
  ...
  # calling the above function
  meanies[b,] <- mean_function(ObsData = bootData)

  # keep track of the iterations completed
  print(b)
}

unadjusted_mean <- mean_function(ObsData)

save(meanies, 
     file = "meanies.rdata")

# CIs for mean
create.CI(pt=unadjusted_mean,
          boot=meanies[[1]])

```

```{r KATIE calculate ICs} 
...
##########################
  ...
  # counts
  length(gHatAW[gHatAW > 0.975])
  length(gHatAW[gHatAW < 0.025])
  
  length(gHatAW[gHatAW > 0.95])
  length(gHatAW[gHatAW < 0.05])
  
  length(gHatAW[gHatAW > 0.9])
  length(gHatAW[gHatAW < 0.1])
  ...
  # bound gn at 0.025, 0.975
  gHatAW_025_975 <- gHatAW
  nrow(gHatAW[gHatAW > 0.975])
  gHatAW_025_975[gHatAW_025_975 > 0.975] <- 0.975
  nrow(gHatAW_025_975[gHatAW_025_975 < 0.025])
  gHatAW_025_975[gHatAW_025_975 < 0.025] <- 0.025
  
  # bound gn at 0.05, 0.95
  gHatAW_05_95 <- gHatAW
  gHatAW_05_95[gHatAW_05_95 > 0.95] <- 0.95
  nrow(gHatAW_05_95[gHatAW_05_95 > 0.95])
  gHatAW_05_95[gHatAW_05_95 < 0.05] <- 0.05
  nrow(gHatAW_05_95[gHatAW_05_95 < 0.05])
  ...
  # counts
  length(gHat0W[gHat0W > 0.975])
  length(gHat0W[gHat0W < 0.025])
  
  length(gHat0W[gHat1W > 0.975])
  length(gHat0W[gHat1W < 0.025])
  
  length(gHat0W[gHat0W > 0.95])
  length(gHat0W[gHat0W < 0.05])
  
  length(gHat0W[gHat1W > 0.95])
  length(gHat0W[gHat1W < 0.05])
  
  length(gHat0W[gHat0W > 0.90])
  length(gHat0W[gHat0W < 0.10])
  
  length(gHat0W[gHat1W > 0.90])
  length(gHat0W[gHat1W < 0.10])
...
# TMLE inference -> p-value + CI assuming normal distribution
  IC <- H.AW*(dataset_complete_cases$math_standard_met/100 - QbarAW.star) + Qbar1W.star - Qbar0W.star - estimates$PsiHat.TMLE
  var(IC)
  (IC_se <- sqrt(var(IC)/n))
  (Psy.LL <- (estimates$PsiHat.TMLE + qnorm(0.05/2, lower.tail = T)*IC_se)*100)
  (Psy.UL <- (estimates$PsiHat.TMLE + qnorm(0.05/2, lower.tail = F)*IC_se)*100)
  2*pnorm(abs(estimates$PsiHat.TMLE/IC_se),lower.tail=F)
# IPTW inference -> p-value + CI assuming normal distribution
  IC <- H.AW*(dataset_complete_cases$math_standard_met/100 - QbarAW.star) + Qbar1W.star - Qbar0W.star - estimates$PsiHat.IPTW
  var(IC)
  (IC_se <- sqrt(var(IC)/n))
  (Psy.LL <- (estimates$PsiHat.IPTW + qnorm(0.05/2, lower.tail = T)*IC_se)*100)
  (Psy.UL <- (estimates$PsiHat.IPTW+qnorm(0.05/2, lower.tail = F)*IC_se)*100)
  2*pnorm(abs(estimates$PsiHat.IPTW/IC_se),lower.tail=F)
# IPTW_HT inference -> p-value + CI assuming normal distribution
  IC <- H.AW*(dataset_complete_cases$math_standard_met/100 - QbarAW.star) + Qbar1W.star - Qbar0W.star - estimates$PsiHat.HW.IPTW
  var(IC)
  (IC_se <- sqrt(var(IC)/n))
  Psy.LL <- (estimates$PsiHat.HW.IPTW + qnorm(0.05/2, lower.tail = T)*IC_se)*100
  Psy.UL <- (estimates$PsiHat.HW.IPTW + qnorm(0.05/2, lower.tail = F)*IC_se)*100
  2*pnorm(abs(estimates$PsiHat.HW.IPTW/IC_se),lower.tail=F)
...
ggplot(data=plot_df) + 
  geom_histogram(aes(ps1),
                 bins=30, 
                 stat = "bin") + 
  facet_wrap(~type) + 
  coord_flip() + 
  theme_bw() +
  xlab(paste0("Probability of exposure \U011D(A=1|W)")) +
  ylab("Count") +
  theme(text=element_text(family="Raleway"))

ggsave(device = "bmp", 
       filename = file.path("output_for_writeups", 
                            "propensity_score.bmp"), 
       height = 5, 
       width = 5)
...
# make stabilized weights
ga <- rep(NA, 8838)

ga[ObsData$A == 1] <- mean(ObsData$A==1)
ga[ObsData$A == 0] <- mean(ObsData$A==0)
...
```{r KATIE make histograms}

# load all the estimates 
load(file = file.path("big_run_one.rdata"))
estimates1 <- estimates

load(file = file.path("estimates_seed_5.rdata"))
estimates2 <- estimates

load(file = file.path("estimates_seed_6.rdata"))
estimates3 <- estimates

load(file = file.path("estimates_seed_7.rdata"))
estimates4 <- estimates

names(estimates1) <- c("SimpSubs", 
                       "IPTW",
                       "IPTW.HT", 
                       "TMLE", 
                       "PsiHat.IPTW.025.975", 
...
estimates_all <- bind_rows(estimates1,
                           estimates2,
                           estimates3,
                           estimates4)

save(estimates_all, 
     file = file.path("estimates_all.rdata"))


# saving the histograms as a pdf
pdf(file="cifp_bootstraps.pdf")

par(mfrow=c(4, 1))

hist(estimates_all[,1],
     main = "Histogram of point estimates from the Simple Substitution estimator over 500 bootstrapped samples",
     xlab = "Point Estimates")

hist(estimates_all[,2],
     main = "Histogram of point estimates from IPTW estimator over 500 bootstrapped samples", 
     xlab="Point Estimates")

hist(estimates_all[,3], 
     main="Histogram of point estimates from IPTW HT over 500 bootstrapped samples", 
     xlab="Point Estimates")

hist(estimates_all[,4], 
     main="Histogram of point estimates from TMLE over 500 bootstrapped samples", 
     xlab="Point Estimates")
dev.off()

pdf(file="cifp_truncated_bootstraps.pdf")

par(mfrow=c(3, 2))

hist(estimates_all[,5], 
     main="Histogram of point estimates from IPTW truncated to gn 0.025, 0.975 over 500 bootstrapped samples", 
     xlab="Point Estimates")

hist(estimates_all[,6], 
     main="Histogram of point estimates from IPTW truncated to gn 0.05, 0.95 over 500 bootstrapped samples", 
     xlab="Point Estimates")

hist(estimates_all[,7], 
     main="Histogram of point estimates from IPTW truncated to gn 0.10, 0.90 over 500 bootstrapped samples", 
     xlab="Point Estimates")

hist(estimates_all[,8], 
     main="Histogram of point estimates from TMLE truncated to gn 0.025, 0.975 over 500 bootstrapped samples", 
     xlab="Point Estimates")

hist(estimates_all[,9], 
     main="Histogram of point estimates from TMLE truncated to gn 0.05, 0.95 over B bootstrapped samples", 
     xlab="Point Estimates")

hist(estimates_all[,10], 
     main="Histogram of point estimates from TMLE truncated to gn 0.10, 0.90 over B bootstrapped samples", 
     xlab="Point Estimates")
...
```{r KATIE cross-validating SuperLearner for g and Q}
...
# this is the parallel equivalent of a seed (CHANGE EVERY TIME SO HELP YOU GAIA)
parallel::clusterSetRNGStream(cluster, 8)
...
little_data <- ObsData[sample(1:nrow(ObsData), 
                              1000, 
                              replace=F),]

# #########################
# # RUN CV SuperLearner for Q on little data!
# #########################
# 
# # CV.superlearner for Q
# X <- little_data[,names(little_data) %in% c(adjustment_set_variables, "A")]
#   
# # set the exposure = 1 in X1 and the exposure = 0 in X0
# X1 <- X0 <- X
...
# X0$A <- 0 # under control
#   
# CV.SL.out <- CV.SuperLearner(Y = little_data$Y, 
#                              X = X, 
#                              SL.library = Q.SL.library,
#                              family = "quasibinomial",
#                              parallel = cluster,
...
#########################
# RUN CV SuperLearner for Q on big data!
#########################
...
Q.CV.SL.out <- CV.SuperLearner(Y = ObsData$Y, 
                               X = X, 
                               SL.library = Q.SL.library,
                               family = "quasibinomial",
                               parallel = cluster,
...
#########################
# RUN CV SuperLearner for g
#########################

g.CV.SL.out <- CV.SuperLearner(Y = ObsData$A, 
                               X = subset(ObsData, 
                                          select = -c(A, Y)),
                               SL.library = g.SL.library, 
                               family = "binomial", 
...
save(Q.CV.SL.out, file = "Q_CV_SL_out.rdata")

save(g.CV.SL.out, file = "g_CV_SL_out.rdata")
...
var(ltmle_tmle$IC.iptw[,2])
(IC_se <- sqrt(var(ltmle_tmle$IC.iptw[,2])/nrow(ltmle_data)))
(Psy.LL <- ltmle_tmle$beta.iptw[2]-1.96*IC_se)
(Psy.UL <- ltmle_tmle$beta.iptw[2]+1.96*IC_se)
2*pnorm(abs(ltmle_tmle$beta.iptw[2]/IC_se),lower.tail=F)
...
```{r, histograms from bootstraps}
load(file = "estimates_all.rdata")
load(file = "g_CV_SL_out.rdata")
load(file = "Q_CV_SL_out.rdata")
load(file = "meanies.rdata")

######################
## bootstraps first ##
######################
names(estimates_all)
summary(estimates_all)

estimates_all_x100 <- estimates_all*100
summary(estimates_all_x100)

apply(estimates_all_x100,2,function(x) hist(x,breaks=20))
for (i in 1:ncol(estimates_all_x100)) {
  p <- ggplot(estimates_all_x100, aes(x=estimates_all_x100[,i])) + 
    geom_density(color="darkblue", y=..scaled.., fill="lightblue") + 
    geom_vline(aes(xintercept=mean(estimates_all_x100[,i])),color="blue", linetype="dashed", size=1) +
    labs(title=paste0(names(estimates_all_x100[i])," density curve"),x=names(estimates_all_x100[i]), y="Density") +
  theme_classic(base_size=20)
  print(p)
}

make_density_curve <- function(data,variable,xlab,ylab,title,line,line.at) {
  p <- ggplot(data, aes(x=data[,variable])) + 
    geom_histogram(aes(y=..density..), colour="darkblue", fill="white") +
    geom_density(alpha=.6,color="darkblue",fill="lightblue") + 
    labs(title=title,x=xlab, y=ylab) +
  theme_classic(base_size=20)
  if (line==TRUE) {p <- p + geom_vline(aes(xintercept=line.at),color="blue", linetype="dashed", size=1)}
  print(p)
}

names(estimates_all_x100)
make_density_curve(estimates_all_x100,"SimpSubs","Estimates","Density","",line==TRUE,line.at=0.40)
make_density_curve(estimates_all_x100,"IPTW","Estimates","Density","",line.at=13.32)
make_density_curve(estimates_all_x100,"IPTW.HT","Estimates","Density","",line.at=4.58)
make_density_curve(estimates_all_x100,"TMLE","Estimates","Density","",line.at=0.64)

gridExtra::grid.arrange(
make_density_curve(estimates_all_x100,"IPTW","Estimates","Density","Regular IPTW",line.at=13.32),
make_density_curve(estimates_all_x100,"PsiHat.IPTW.025.975","Estimates","Density","Truncated at {.025,.975}",line.at=13.32),
make_density_curve(estimates_all_x100,"PsiHat.IPTW.05.95","Estimates","Density","Truncated at {.05,.95}",line.at=13.33),
make_density_curve(estimates_all_x100,"PsiHat.IPTW.10.90","Estimates","Density","Truncated at {.10,.90}",line.at=13.41),
nrow=2,ncol=2
)

gridExtra::grid.arrange(
make_density_curve(estimates_all_x100,"TMLE","Estimates","Density","Regular TMLE",line.at=0.64),
make_density_curve(estimates_all_x100,"PsiHat.TMLE.trunc.025.975","Estimates","Density","Truncated at {.025,.975}",line.at=0.64),
make_density_curve(estimates_all_x100,"PsiHat.TMLE.trunc.05.95","Estimates","Density","Truncated at {.05,.95}",line.at=0.64),
make_density_curve(estimates_all_x100,"PsiHat.TMLE.trunc.10.90","Estimates","Density","Truncated at {.10,.90}",line.at=0.63),
nrow=2,ncol=2
)

names(meanies)
summary(meanies)
meanies_x100 <- meanies*100
make_density_curve(meanies_x100,"matrix.NA..nrow...B..ncol...1.","Estimates","Density","",line.at=-2.33)
...
############################################
## histogram of pollution and test scores ##
############################################
names(dataset_complete_cases)
make_density_curve(data=dataset_complete_cases,
                   variable="modeled_air_at_school",
                   xlab=expression(paste("Mean PM"["2.5"]," concentration (",mu,"g/m"^"3",") in 2018")),
                   ylab="Density",
                   title="",
                   line=TRUE,
                   line.at=12)
make_density_curve(data=dataset_complete_cases,
                   variable="math_standard_met",
                   xlab="Mean Percentage of Students Who Met or Exceeded \nMath Standard in 2018-19",
                   ylab="Density",
                   title="",
                   line=FALSE)
## plus a violin plot
p <- ggplot(dataset_complete_cases, aes(x=pm25_12_plus, y=math_standard_met)) + 
  geom_violin(trim=FALSE, colour="darkblue", fill="darkblue", alpha=.2) + 
  labs(x=expression(paste("> 12 PM"["2.5"]," concentration (",mu,"g/m"^"3",") in 2018")),
       y="Mean Percentage of Students Who Met or Exceeded \nMath Standard in 2018-19") + 
  geom_dotplot(binaxis='y', stackdir='center', dotsize=.1, binwidth=0) +
  geom_jitter(shape=16, position=position_jitter(0.2), alpha=1, colour="mediumorchid2") + 
  geom_boxplot(width=0.1, colour="darkblue", outlier.shape = NA) + 
  ylim(0,100) + 
  theme_classic(base_size=20)
p
```
