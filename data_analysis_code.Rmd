...
```{r KATIE code pre-SuperLearner, simple GLMs + descriptive work}
...
```{r plots for slides, scatter & box plots bw PM2.5 & other variables}
...
```{r KATIE tabling the ancestors, Table 1}
...
```{r old code not to run, simplifying complex algorithms}
...
```{r KATIE sad wrapper failures, feel this section so hard; those contXcont itx so hard}
...
save(estimates, file = "estimates_seed_7.Rdata")
...
# specify SL library
  ...
  SL.library <- c("SL.mean","SL.glm","SL.glm.interaction","SL.bayesglm","SL.rpartPrune"
...
end_time <- Sys.time()
(((end_time - start_time)/2)*500)/60 # should take about 5 hours to run
...
make_density_curve <- function(data,variable,xlab,ylab,title) {
  p <- ggplot(data, aes(x=data[,variable])) + 
    ...
    geom_vline(aes(xintercept=mean(data[,variable])),color="blue", linetype="dashed", size=1) +
...
make_density_curve(estimates_all_x100,"SimpSubs","Estimates","Density","")
make_density_curve(estimates_all_x100,"IPTW","Estimates","Density","")
make_density_curve(estimates_all_x100,"IPTW.HT","Estimates","Density","")
make_density_curve(estimates_all_x100,"TMLE","Estimates","Density","")

gridExtra::grid.arrange(
make_density_curve(estimates_all_x100,"IPTW","Estimates","Density","Regular IPTW"),
make_density_curve(estimates_all_x100,"PsiHat.IPTW.025.975","Estimates","Density","Truncated at {.025,.975}"),
make_density_curve(estimates_all_x100,"PsiHat.IPTW.05.95","Estimates","Density","Truncated at {.05,.95}"),
make_density_curve(estimates_all_x100,"PsiHat.IPTW.10.90","Estimates","Density","Truncated at {.10,.90}"),
nrow=2,ncol=2
)

gridExtra::grid.arrange(
make_density_curve(estimates_all_x100,"TMLE","Estimates","Density","Regular TMLE"),
make_density_curve(estimates_all_x100,"PsiHat.TMLE.trunc.025.975","Estimates","Density","Truncated at {.025,.975}"),
make_density_curve(estimates_all_x100,"PsiHat.TMLE.trunc.05.95","Estimates","Density","Truncated at {.05,.95}"),
make_density_curve(estimates_all_x100,"PsiHat.TMLE.trunc.10.90","Estimates","Density","Truncated at {.10,.90}"),
nrow=2,ncol=2
)
```
